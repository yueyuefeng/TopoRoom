...
**每次构建升版本；签名固定.** Every export runs
`./godot/scripts/bump_android_version.sh` (`versionCode` +1 and patch
`versionName`, e.g. 0.1.0 → 0.1.1) and signs with the committed keystore
`godot/android/keystore/debug.keystore`. Do **not** let the editor generate a
new debug key. See [android/keystore/README.md](./android/keystore/README.md).
...
1. Godot 4.3+ **Editor Settings → Export → Android**
   ...
   - Debug keystore = **project** `android/keystore/debug.keystore` (password `android`, alias `androiddebugkey`)
...
5. **Project → Export → Android → Export Debug APK**, or:
   ...
   Equivalent version bump: `./scripts/bump_android_version.sh`.

`export_credentials.cfg` is generated by `wire_android_signing.sh` (absolute
path to the committed keystore) and is gitignored. Do not regenerate the key.
